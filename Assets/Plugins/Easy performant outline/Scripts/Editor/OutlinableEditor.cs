...
namespace EPOOutline
{
    ...
    public class OutlinableEditor : Editor
    {
        ...
        private void CheckList(SerializedProperty targets)
        {
            ...
            targetsList.drawHeaderCallback = position =>
                EditorGUI.LabelField(position, "Renderers. All renderers that has to be outlined must be included here.");
            ...
            targetsList.onAddDropdownCallback = (buttonRect, targetList) =>
            {
                ...
                if (!Application.isPlaying)
                {
                    menu.AddItem(new GUIContent("Add all renderers"), false, () =>
                    {
                        ...
                        EditorUtility.SetDirty(outlinable);
                    });
                    ...
                    {
                        outlinable.AddAllChildRenderersToRenderingList(RenderersAddingMode.SkinnedMeshRenderer);
                        EditorUtility.SetDirty(outlinable);
                    });
                    ...
                    {
                        outlinable.AddAllChildRenderersToRenderingList(RenderersAddingMode.SpriteRenderer);
                        EditorUtility.SetDirty(outlinable);
                    });
                }
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("Add All Child Renderers"))
            {
                foreach (var obj in targets)
                {
                    if (obj is Outlinable outlinable)
                    {
                        Undo.RecordObject(outlinable, "Add All Child Renderers");
                        outlinable.AddAllChildRenderersToRenderingList(RenderersAddingMode.All);
                        EditorUtility.SetDirty(outlinable);
                    }
                }
