...
public class ListEvent : MonoBehaviour {
    ...
    public void OnClick() {
		...
		transform.parent.parent.Find ("Button").gameObject.SetActive(false);
    ...
    IEnumerator downAsFile(string path, string filename) {                              //path-服务器上的下载路径 filename-下载到的本地路径 
        ...
        if (www.error == null) {                                                        //如果解压完成
			...
			transform.parent.parent.Find ("Over").gameObject.SetActive(true);		//打开完成提示
			Debug.Log("结束下载资源包");
			gameObject.GetComponent<Image> ().color = new Color (0.67f, 0.55f, 0.55f, 1.0f);
			transform.Find ("Image").GetComponent<Image>().color = new Color (0.67f, 0.55f, 0.55f, 1.0f);
			transform.Find ("Text").GetComponent<Text> ().color = new Color (0.56f, 0.49f, 0.49f, 1.0f);
			transform.Find ("Over").gameObject.SetActive (true);
