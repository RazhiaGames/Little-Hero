...
public class InitialEXList : MonoBehaviour {
	...
	public void initialPrefab(string[] str, int i, bool type) {
        ...
        temp.transform.Find("Text").transform.GetComponent<Text>().text = str[1];				//更改资源包名称（顽皮的指针）
		...
		temp.transform.SetParent(Canvas.transform.Find("ButtonGroup"));  	        			//设为ButtonGroup的子对象
		...
		if(!type) {																					//如果已经是已经下载的实验数据包
			...
			temp.transform.Find ("Image").GetComponent<Image>().color = 
				new Color (0.67f, 0.55f, 0.55f, 1.0f);												//子物体Image颜色变灰
			temp.transform.Find ("Text").GetComponent<Text> ().color = 
				new Color (0.56f, 0.49f, 0.49f, 1.0f);												//子物体Text颜色变灰
			temp.transform.Find ("Over").gameObject.SetActive (true);							//打开‘已下载’提示字样
