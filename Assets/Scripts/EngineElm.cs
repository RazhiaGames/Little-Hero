...
public class EngineElm : CircuitElm {
    ...
    IEnumerator Rot() {
        ...
        newFan.GetComponent<Rigidbody>().linearVelocity = new Vector3(randomX, 10 + Mathf.Abs(runCurrent) * 10, randomZ);
        ...
        while (newFan.GetComponent<Rigidbody>().linearVelocity != Vector3.zero) {    //若还有速度
            if (runCurrent > 0) {
