...
public class Move : MonoBehaviour {
	...
	void DesktopInput() {
		...
		if (  mx!= 0 || my !=0 ) {
			...
			if (Input.GetMouseButton(0)) {
				// 摄像机移动
				transform.Find ("Main Camera").Translate(new Vector3(mx*Time.deltaTime, my * Time.deltaTime, 0));
			}
		}
	}
					...
					if (i == 0) {														// 如果是第一支手指
						...
						if (Vector2.Angle (mov1, mov2) <= 90 && (mov1.magnitude > 0.8f || mov2.magnitude > 0.8f)) {
							if(Vector2.Angle(mov1, new Vector2(1, 0)) < 10) {			// 滑动方向向量如果和x正轴向量夹角小于10度
								...
								//垂直向上旋转
								transform.Find ("Main Camera").Rotate (-RotateVSpeed * Time.deltaTime, 0, 0);
							} else if(Vector2.Angle(mov1, new Vector2(0, -1)) < 10) {	// 滑动方向向量如果和y负轴向量夹角小于10度
								// 垂直向下旋转
								transform.Find ("Main Camera").Rotate (RotateVSpeed * Time.deltaTime, 0, 0);
							}
    ...
    private void setUICamera(CircuitElm temp, GameObject UICamera) {
        ...
        Magnifier.transform.Find("MagnifierChild").gameObject.SetActive(true);
        Magnifier.transform.Find("MagnifierChild").gameObject.GetComponent<Magnifier>().reset();
