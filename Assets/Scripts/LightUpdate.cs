...
public class LightUpdate : MonoBehaviour {
		...
		for (int i = 0; i < Elms.Length; i++) {
			if (Elms [i].transform.name.Length > 23) {
				if(Elms[i].name.Substring(0, 25).Equals("PhotosensitiveResistorElm")) {
					...
					PhotosensitiveResistorElm[count++].transform.Find ("Spotlight").gameObject.SetActive (true);
				}
			}
		}
	...
	void Update () {
		...
		for (int i = 0; i < PhotosensitiveResistorElm.Length; i++) {
			...
			PhotosensitiveResistorElm[i].transform.Find ("Spotlight").GetComponent<Light> ().intensity = 8.0f * value;
